  ...
  return (
    <main className="container mx-auto">
      <div className="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
        ...
        {/* Title */}
        <h2 className="text-3xl font-bold tracking-tight text-center text-white mb-6">
          My Portfolio
        </h2>
        ...
        <div className="mb-10 flex justify-center space-x-6">
          <Link href="/portofolio" 
            className={`px-4 py-2 rounded-lg transition ${
              !category ? 'bg-[#3b2b23] text-white shadow-md' : 'text-gray-700 hover:text-yellow-300'
            }`}
          >
            All
          </Link>
        ...
        <div className="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
          ...
          {filteredPorto.map((porto, index) => (
            <div key={index} className="group relative bg-yellow-950 rounded-xl p-4 shadow-xl border border-[#3b2b23]/40 hover:border-white/20 transition">

              <Link href={`/portofolio/${porto.id}`}>

                {/* Image Card */}
                <div className="overflow-hidden rounded-lg bg-black/20 group-hover:opacity-90 transition lg:h-56 h-48">
                  <Image
                    alt={porto.name}
                    src={porto.image}
                    width={300}
